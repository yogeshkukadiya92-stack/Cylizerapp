...
export class PostgresCalloraRepository implements CalloraRepository {
  ...
  async issueReportDownloadToken(options: { organizationId: OrganizationId; userId: string; jobId: string; tokenHash: Uint8Array; expiresAt: string; at: string }): Promise<boolean> {
    if(!isCanonicalUuid(options.jobId)||options.tokenHash.length!==32)return false;
    return this.withTenant(options.organizationId,options.userId,async(client)=>{const result=await client.query<DbRow>(`update callora.report_export_jobs job set download_token_hash=$4::bytea,download_expires_at=$5::timestamptz where job.organization_id=$1::uuid and job.id=$2::uuid and job.requested_by_user_id=$3::uuid and job.status='ready' and not exists(select 1 from callora.report_download_redemptions redemption where redemption.organization_id=job.organization_id and redemption.report_export_job_id=job.id) returning job.id`,[options.organizationId,options.jobId,options.userId,Buffer.from(options.tokenHash),options.expiresAt]);return result.rows.length===1;});
  }
