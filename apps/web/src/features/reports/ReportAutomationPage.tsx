...
export function ReportAutomationPage({ authSession, client, onAuthenticationFailure, onNotify }: Props) {
  ...
  const [scheduleDraft, setScheduleDraft] = useState({ name: '', savedViewId: demoSavedViews[0]!.id, cadence: 'daily' as 'daily'|'weekly', weekDay: 1, localTime: '08:00', format: 'pdf' as 'csv'|'xlsx'|'pdf', recipients: '' })
  ...
  const queueExport = async () => { try { const accessToken=await token(); if(!client||!accessToken){onNotify('Export queued');return} const job=await client.createReportExport({kind:'lead_performance',format:'xlsx',parameters:{period:'this_month'}},accessToken); setJobs((current)=>[job,...current]); onNotify('Export queued') } catch { onNotify('Could not queue export') } }
  return <div className="report-automation-page">
    <section aria-label="Report automation metrics" className="automation-metrics">
      ...
      <article><Download/><span>Ready downloads<strong>3</strong></span></article>
